...
- **Corpus (MVP):** (a) Roberto's **public site content** (Work timeline, About, Lab write-ups) + (b) a small **curated "facts & FAQ" doc** Roberto maintains (availability, roles he's open to, skills, the *kinds* of client problems he solves, how he works). Nothing else.
...
**v2 `[2]`:** agent-readability surface (`llms.txt` / public ask-roberto endpoint) · conversation memory/persistence · persona-adaptive UX beyond the CTA · analytics dashboard · multi-language (Spanish) · calendar/availability integration · "share this conversation."
...
- **Evals:** a **golden Q&A set** (P0 questions + must-refuse/NDA traps) with a **groundedness pass bar = 0 fabrications**; run before ship and on flow changes (Arize). *Eval pass is a release gate, not a nice-to-have.*
